-- ============================================================================
-- ENTERPRISE USER RESET - BULLETPROOF VERSION (FIXED)
-- This script performs a comprehensive user reset for enterprise deployment
-- Fixed to use TEXT for role column instead of user_role enum
-- ============================================================================
...
BEGIN
    ...
    RAISE NOTICE 'Current total profiles: %', v_total_profiles;
    RAISE NOTICE 'Current active profiles: %', v_active_profiles;
    
    -- If we already have 5 active users, just update passwords
    IF v_active_profiles = 5 THEN
        RAISE NOTICE '✅ Found exactly 5 active users - updating passwords only';
        ...
        LOOP
            ...
            RAISE NOTICE 'Updated user: % (role: %)', v_user_record.email, v_user_record.role;
        END LOOP;
        
    ELSE
        -- Perform full reset process
        RAISE NOTICE '⚠️ User count is not 5, performing full reset...';
        ...
        LOOP
            ...
            -- Insert or update user (removed ::user_role cast, using TEXT)
            INSERT INTO profiles (
                ...
                v_user_record.role,  -- Using TEXT instead of user_role enum
    ...
    IF v_verification_passed THEN
        ...
        RAISE NOTICE '🛡️  Enterprise-grade security is now active';
    ELSE
