-- Final Missing Table: emergency_escalations (UNIVERSAL VERSION)
-- SAMIA TAROT Platform Database Schema - Complete the last 1%!
-- This version detects which columns exist and uses the correct ones
...
-- Create RLS policy that works with different column structures
DO $$
DECLARE
    has_user_id BOOLEAN := FALSE;
    has_client_id BOOLEAN := FALSE;
    has_reader_id BOOLEAN := FALSE;
BEGIN
    -- Check which columns exist in emergency_call_logs
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'emergency_call_logs' AND column_name = 'user_id'
    ) INTO has_user_id;
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'emergency_call_logs' AND column_name = 'client_id'
    ) INTO has_client_id;
    
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'emergency_call_logs' AND column_name = 'reader_id'
    ) INTO has_reader_id;
    
    -- Create policies based on available columns
    IF has_client_id AND has_reader_id THEN
        -- Version with client_id and reader_id
        EXECUTE 'CREATE POLICY "Users can view related escalations (client_id)" ON emergency_escalations
            FOR SELECT USING (
                ...
                EXISTS (
                    ...
                    AND (ecl.client_id = auth.uid() OR ecl.reader_id = auth.uid())
                )
            )';
    ELSIF has_user_id AND has_reader_id THEN
        -- Version with user_id and reader_id
        EXECUTE 'CREATE POLICY "Users can view related escalations (user_id)" ON emergency_escalations
            FOR SELECT USING (
                auth.uid() = escalated_from OR 
                auth.uid() = escalated_to OR
                EXISTS (
    ...
    ELSIF has_user_id THEN
        -- Version with only user_id
        EXECUTE 'CREATE POLICY "Users can view related escalations (user_only)" ON emergency_escalations
            FOR SELECT USING (
                auth.uid() = escalated_from OR 
                auth.uid() = escalated_to OR
                EXISTS (
    ...
    ELSIF has_client_id THEN
        -- Version with only client_id
        EXECUTE 'CREATE POLICY "Users can view related escalations (client_only)" ON emergency_escalations
            FOR SELECT USING (
                auth.uid() = escalated_from OR 
                auth.uid() = escalated_to OR
                EXISTS (
                    SELECT 1 FROM emergency_call_logs ecl 
                    WHERE ecl.id = emergency_call_log_id 
                    AND ecl.client_id = auth.uid()
                )
            )';
    ELSE
        -- Fallback - minimal policy
        EXECUTE 'CREATE POLICY "Users can view own escalations" ON emergency_escalations
            FOR SELECT USING (
                auth.uid() = escalated_from OR 
    ...
    RAISE NOTICE 'RLS Policy created for emergency_escalations';
    RAISE NOTICE 'Detected columns: user_id=%, client_id=%, reader_id=%', has_user_id, has_client_id, has_reader_id;
END $$;
...
-- Function to create emergency escalation (works with any column structure)
...
BEGIN
    ...
    RAISE NOTICE '   ✅ RLS policies configured (UNIVERSAL - auto-detects columns)';
