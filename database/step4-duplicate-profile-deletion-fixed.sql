-- =====================================================================================
-- STEP 4: DUPLICATE PROFILE DELETION (FIXED VERSION)
-- =====================================================================================
-- Version: 3.1 - Fixed version that doesn't rely on temporary audit log
...
-- FIX: Removed dependency on temporary audit log for prerequisite validation
-- =====================================================================================
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM script_config WHERE key = 'MAIN_ADMIN_ID') THEN
        -- Recreate configuration (user confirmed Step 3 was successful)
        INSERT INTO script_config (key, value, description) VALUES
            ...
            ('SCRIPT_VERSION', '3.1', 'Script version for audit purposes');
...
BEGIN
    -- Step 4: Duplicate Profile Deletion
    RAISE NOTICE '==========================================';
    RAISE NOTICE 'STEP 4: DUPLICATE PROFILE DELETION (FIXED)';
    RAISE NOTICE '==========================================';
    ...
    -- REMOVED: Strict audit log check (unreliable with temporary tables)
    -- Instead: Comprehensive safety checks will verify system state
    ...
    RAISE NOTICE '==========================================';
    RAISE NOTICE 'COMPREHENSIVE SAFETY VERIFICATION:';
    RAISE NOTICE '==========================================';
    ...
    IF duplicate_count = 0 THEN
        ...
        RAISE NOTICE '==========================================';
        RAISE NOTICE 'STEP 4 COMPLETE: ALREADY CLEANED';
        RAISE NOTICE '==========================================';
        RAISE NOTICE '✅ Duplicate profile was already removed';
    ...
    -- Safety Check 3: Comprehensive foreign key verification
    RAISE NOTICE '==========================================';
    RAISE NOTICE 'COMPREHENSIVE FOREIGN KEY VERIFICATION:';
    RAISE NOTICE '==========================================';
        ...
        IF table_exists(rec.table_name) AND column_exists(rec.table_name, rec.column_name) THEN
            BEGIN
                ...
                IF duplicate_count > 0 THEN
                    ...
                    RAISE NOTICE '⚠️ CRITICAL: Found % remaining references in %.%', 
    ...
    IF foreign_key_count > 0 THEN
        RAISE EXCEPTION 'CRITICAL ERROR: Found % remaining foreign key references to duplicate profile. Please run Step 3 again to clean up foreign keys before deletion.', foreign_key_count;
    END IF;
