-- =====================================================
-- SAMIA TAROT - COMPLETE ADVANCED ADMIN FEATURES V2 (BULLETPROOF FINAL)
-- =====================================================
-- مضمون نهائياً - فحص كامل للجداول والأعمدة قبل أي عملية
...
-- 6️⃣ DOCUMENTATION & ONBOARDING (🔥 الجدول اللي كان يطلع الخطأ)
...
-- 🔥 التأكد من أعمدة documentation_entries (الجدول اللي كان يطلع الخطأ)
ALTER TABLE documentation_entries ADD COLUMN IF NOT EXISTS page_key VARCHAR(100) UNIQUE;
ALTER TABLE documentation_entries ADD COLUMN IF NOT EXISTS title VARCHAR(255);
ALTER TABLE documentation_entries ADD COLUMN IF NOT EXISTS content TEXT;
...
-- =====================================================
-- STEP 5: INSERT DEFAULT DATA (🔥 WITH COLUMN VERIFICATION)
-- =====================================================
...
-- 🔥 INSERT DOCUMENTATION ENTRIES مع فحص وجود العمود page_key (السطر اللي كان يطلع الخطأ!)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '🎉 SAMIA TAROT ADVANCED ADMIN SETUP COMPLETE (BULLETPROOF FINAL)!';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '✅ All INSERT operations with column verification';
    ...
    RAISE NOTICE '🔥 Bulletproof final version - handles ALL edge cases!';
    RAISE NOTICE '💎 No more "page_key does not exist" or any column errors!';
    RAISE NOTICE 'مضمون 100% للـ production! 😉';
    RAISE NOTICE '';
END $$; 