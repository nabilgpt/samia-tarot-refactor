-- Final Missing Table: emergency_escalations (SIMPLE VERSION)
-- SAMIA TAROT Platform Database Schema - Complete the last 1%!
-- Simple version that works with basic emergency_call_logs structure
...
-- Simple RLS policy - users can view escalations they are directly involved in
CREATE POLICY "Users can view own escalations" ON emergency_escalations
    FOR SELECT USING (
...
-- Function to create emergency escalation (simple version)
...
BEGIN
    ...
    IF p_escalated_to IS NULL THEN
        -- Try to find available reader from reader_availability table
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            -- If reader_availability doesn't exist, just find any admin/reader
            SELECT id INTO available_responder
            FROM profiles
            WHERE role IN ('admin', 'super_admin', 'reader')
            AND id != p_escalated_from
            ORDER BY 
    ...
    -- Send notification if escalated to someone (if notifications table exists)
    IF p_escalated_to IS NOT NULL THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            -- If notifications table doesn't exist or has issues, continue anyway
            NULL;
        END;
    END IF;
...
BEGIN
    ...
    RAISE NOTICE '   ✅ RLS policies configured (SIMPLE VERSION)';
