...
const UnifiedDashboardLayout = ({ 
  ...
  return (
    <MainLayout showNavbar={false} className="lg:pl-72 rtl:lg:pl-0 rtl:lg:pr-72">
      ...
      <div className={`hidden lg:fixed lg:top-0 ${language === 'ar' ? 'lg:right-0' : 'lg:left-0'} lg:z-30 lg:w-72 lg:h-full lg:block`}>
        <SidebarContent />
      </div>
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div
            ...
            animate="open"
            exit="closed"
            className={`lg:hidden fixed top-0 ${language === 'ar' ? 'right-0' : 'left-0'} z-50 w-72 h-full`}
          >
      ...
      {/* Desktop header */}
      <div className="hidden lg:block sticky top-0 z-20 bg-gray-900/80 backdrop-blur-md border-b border-white/10 p-4 ml-72 rtl:ml-0 rtl:mr-72">
        <div className="flex items-center justify-between">
          <h1 className={`text-xl font-bold bg-gradient-to-r ${roleConfig.titleGradient} bg-clip-text text-transparent`}>
            {t('app.name')} {roleConfig.shortTitle}
          </h1>
