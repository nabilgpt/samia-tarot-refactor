...
async function updateUserToSuperAdmin() {
  try {
    ...
    console.log('\n🚀 Next steps:');
    console.log('1. Log out of your app if logged in');
    ...
    console.log('4. Click the dashboard button to access /dashboard/super-admin');

  } catch (error) {
    console.error('❌ Unexpected error:', error.message);
    console.log('\n📋 Fallback: Run this SQL in Supabase Dashboard:');
    console.log(`
      UPDATE profiles 
      SET role = 'super_admin', updated_at = NOW()
      WHERE email = 'info@samiatarot.com';
