-- 017_rls_security_hardening_complete.sql
-- Complete RLS policy gaps and enforce strict row-level security
...
CREATE POLICY "payment_intents_user_access" ON payment_intents
  FOR ALL TO authenticated
  USING (
    user_id = auth.uid() OR
...
DROP POLICY IF EXISTS "invoices_user_access" ON invoices;
...
CREATE INDEX IF NOT EXISTS idx_orders_reader_auth ON orders(assigned_reader) WHERE assigned_reader = auth.uid();
CREATE INDEX IF NOT EXISTS idx_horoscopes_public_access ON horoscopes(scope, ref_date, approved_at)
  WHERE scope = 'daily' AND ref_date = CURRENT_DATE AND approved_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_horoscopes_internal_access ON horoscopes(ref_date)
  WHERE ref_date >= CURRENT_DATE - INTERVAL '60 days';

-- Test RLS policies work correctly
DO $$
BEGIN
  RAISE NOTICE 'RLS Security Hardening Complete - All tables secured with row-level security policies';
END $$;